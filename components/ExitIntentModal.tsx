...
const ExitIntentModal: React.FC<ExitIntentModalProps> = ({ onClose, onApplyPromo }) => {
  ...
  return (
    ...
    >
      ...
      >
        <button onClick={onClose} className="absolute top-4 right-4 p-2 rounded-full hover:bg-gray-200" aria-label="Close">
          <XIcon className="h-6 w-6" />
        </button>
        <GiftIcon className="h-16 w-16 text-brand-primary" />
        <h2 className="text-3xl font-serif font-bold mt-4">Wait, Don't Go!</h2>
        <p className="text-gray-600 mt-2">You've got great taste. Complete your order now and enjoy <span className="font-bold">15% off</span> on us!</p>
        <div className="mt-6 w-full">
            <p className="text-sm text-gray-500">Use code:</p>
            <p className="text-2xl font-bold font-mono tracking-widest bg-brand-accent py-2 px-4 rounded-md my-2">COMEBACK15</p>
            <button
                onClick={handleApply}
