...
export default function CustomCursor() {
  ...
  useGSAP(() => {
    ...
    const onMouseMove = (e: MouseEvent) => {
      ...
      if (cursorState === 'cta') {
        gsap.to(ringRef.current, { width: 80, height: 80, marginLeft: -40, marginTop: -40, borderColor: 'var(--saffron)', duration: 0.3, ease: 'power2.out', overwrite: 'auto' });
      } else if (cursorState === 'link') {
        gsap.to(ringRef.current, { width: 50, height: 50, marginLeft: -25, marginTop: -25, borderColor: 'var(--dust)', duration: 0.3, ease: 'power2.out', overwrite: 'auto' });
      } else {
        gsap.to(ringRef.current, { width: 36, height: 36, marginLeft: -18, marginTop: -18, borderColor: 'var(--mist)', duration: 0.3, ease: 'power2.out', overwrite: 'auto' });
      }
    };
