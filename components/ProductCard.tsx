  ...
  return (
    <article className="bg-white rounded-lg shadow-md overflow-hidden group transform hover:-translate-y-2 transition-all duration-300 border border-transparent hover:border-brand-primary/50 hover:shadow-xl">
      <div className="relative">
        ...
        >
          <img className="h-56 w-full object-cover" src={product.imageUrl} alt={`${product.name} - ${product.description}`} />
      ...
      <div className="p-5">
        <h3 
          onClick={() => onSelectProduct(product)} 
          className="text-xl font-serif font-bold text-brand-dark cursor-pointer hover:text-brand-primary transition-colors"
          tabIndex={0}
        ...
        <div className="flex items-center mt-3 h-[20px]">
          {averageRating ? (
            <>
              <div className="flex" role="img" aria-label={`Rating: ${roundedRating} out of 5 stars`}>
