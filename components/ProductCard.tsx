  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg overflow-hidden group transform hover:-translate-y-3 transition-all duration-500 border border-gray-100 hover:border-brand-primary/40 hover:shadow-2xl flex flex-col relative">
        ...
        <div className="h-72 w-full bg-gradient-to-br from-gray-100 to-gray-200 relative overflow-hidden rounded-t-2xl">
          ...
          {isOutOfStock && (
            <div className="absolute inset-0 bg-white/80 backdrop-blur-sm flex items-center justify-center group-hover:hidden transition-opacity duration-300">
                <span className="bg-gray-900 text-white font-bold py-3 px-6 rounded-xl shadow-lg">Out of Stock</span>
            </div>
          )}
        </div>
      ...
      <div className="p-6 flex flex-col flex-grow bg-gradient-to-b from-white to-gray-50/50">
        <button onClick={() => onSelectProduct(product)} className="w-full text-left group/title" aria-label={`View details for ${product.name}`}>
        ...
        <div className="mt-3">
            <div className="flex items-baseline gap-2">
              <span className={`text-2xl font-bold font-sans transition-all duration-300 ${onSale ? 'text-red-600' : 'text-gray-900 group-hover:text-brand-primary'}`}>
                {hasMultiplePrices && <span className="text-sm font-normal text-gray-500">From </span>}
                ${displayPrice.toFixed(2)}
              </span>
              {onSale && (
                  <span className="text-lg font-sans text-gray-400 line-through">${defaultVariant.price.toFixed(2)}</span>
              )}
            </div>
        </div>
            ...
            <div className="flex items-center h-[24px]">
              {averageRating ? (
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-gray-600 font-medium">({product.reviews.length})</span>
                </div>
              ) : (
