...
const DatabaseSeeder: React.FC = () => {
    ...
    return (
        <div className="fixed bottom-4 right-4 bg-white p-6 rounded-lg shadow-xl border border-gray-200 z-50 max-w-sm">
            ...
            <div className="mb-4">
                <p className="text-sm font-medium text-gray-800">Status: <span className="font-normal">{status}</span></p>
            </div>

            <button
                onClick={seedDatabase}
                disabled={isLoading}
                className={`w-full py-2 px-4 rounded-md text-white font-medium transition-colors ${isLoading ? 'bg-gray-400 cursor-not-allowed' : 'bg-brand-primary hover:bg-brand-dark'
                    }`}
            >
