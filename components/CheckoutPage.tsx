...
const CheckoutPage: React.FC<CheckoutPageProps> = ({
  ...
  // Handler for PIN code lookup and autofill (using real India Post API)
  const handlePinLookup = async (pin: string) => {
    setPinLookupStatus({ loading: true });

    try {
      const result = await lookupPinCodeAsync(pin);
      ...
      setPinLookupStatus({
        loading: false,
        success: false,
        message: 'Failed to lookup PIN code',
      });
    }
  };
      ...
      try {
        ...
        try {
          ...
          if (paymentMethod === 'Cash on Delivery' && isPlaceholderBackend) {
            ...
            setOrderConfirmation(demoOrder);
            onPlaceOrder(demoOrder);
            addToast('Order placed successfully (Demo Mode)!', 'success');
          } else {
            console.error('Re-throwing API error:', apiError);
            throw apiError;
          }
        }
      } catch (_error) {
        console.error('Final Order creation failure:', _error);
