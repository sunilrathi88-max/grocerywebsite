...
const SAMPLE_SUBSCRIPTIONS: Subscription[] = [
  ...
  },
  {
    ...
    plan: 'weekly',
    products: [{ name: 'Kerala Cardamom (50g)', quantity: 1 }],
    status: 'paused',
    nextDelivery: '-',
    totalValue: 399,
  ...
  },
  {
    ...
    plan: 'biweekly',
    products: [{ name: 'Chaat Masala (100g)', quantity: 2 }],
    status: 'cancelled',
    nextDelivery: '-',
    totalValue: 250,
  ...
  },
  {
    ...
    plan: 'monthly',
    products: [{ name: 'Premium Saffron (1g)', quantity: 1 }],
    status: 'active',
...
const SubscriptionsTab: React.FC = () => {
  ...
  // const cancelledCount = subscriptions.filter((s) => s.status === 'cancelled').length; // Unused
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredSubscriptions.map((sub) => (
                <tr key={sub.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span
                      className={`px-2 py-1 rounded-full text-xs font-bold ${getPlanBadgeColor(sub.plan)}`}
                    >
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span
                      className={`px-2 py-1 rounded-full text-xs font-bold ${getStatusBadgeColor(sub.status)}`}
                    >
