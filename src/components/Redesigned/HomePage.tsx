...
import { ArrowRight, Star, ShieldCheck, Zap, Truck, RotateCcw, Leaf, Flame, Box, Sparkles } from 'lucide-react';
...
const HomePage: React.FC<HomePageProps> = ({ products }) => {
  const bestSellers = products.filter(p => p.badge === 'Bestseller' || p.reviews.length > 5).slice(0, 8);
    ...
    <div className="bg-[#FAF6F2] min-h-screen pb-20">
      ...
      <section className="relative overflow-hidden bg-[#42210B] text-white py-20 px-4 md:py-32">
        ...
        <div className="max-w-4xl mx-auto text-center relative z-10">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/10 backdrop-blur-md border border-white/20 mb-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <span className="text-xs font-bold tracking-[0.2em] uppercase">Trusted by 10,000+ Families Since 1965</span>
          </div>
          ...
          <p className="text-lg md:text-xl text-stone-300 mb-12 max-w-2xl mx-auto leading-relaxed font-medium animate-in fade-in slide-in-from-bottom-8 duration-1000 delay-300">
            Single-origin spices from Salem, Mathania & Ramganj Mandi. Cold-ground below 10°C to preserve 40% more aromatic oils.
          </p>
          ...
          <div className="mt-16 flex flex-wrap justify-center gap-x-8 gap-y-4 opacity-70">
            {['❄️ Cold-ground <10°C', '✅ FSSAI Certified', '🌿 Zero fillers', '📍 Single-origin'].map((t, i) => (
              <span key={i} className="text-sm font-bold tracking-wider">{t}</span>
            ))}
          </div>
        </div>
      </section>
          ...
          <div className="flex items-center justify-center md:justify-center gap-8 md:gap-16">
            {REDESIGN_CATEGORIES.map(cat => (
              ...
              >
                ...
                <span className="text-[11px] font-bold text-stone-500 uppercase tracking-widest group-hover:text-[#42210B] transition-colors">{cat.label}</span>
              </Link>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
            <div className="max-w-xl">
              <span className="text-[#B38B59] font-bold tracking-[0.3em] uppercase text-xs mb-4 block">Difference that matters</span>
              <h2 className="font-display text-3xl md:text-5xl font-bold text-[#42210B]">Why Rathi Naturals?</h2>
            </div>
          </div>
          
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              { icon: <Zap className="text-[#B38B59]" />, title: 'Cold-ground <10°C', desc: '40% more volatile oils preserved vs hot-grinding. The science behind better aroma.' },
              { icon: <ShieldCheck className="text-[#B38B59]" />, title: 'Single-origin only', desc: 'Salem turmeric. Mathania chilli. Ramganj Mandi coriander. Always named, never blended.' },
              { icon: <Zap className="text-[#B38B59]" />, title: 'Total Purity', desc: 'Zero fillers, starch, or artificial coloring. Lab-tested for heavy metals & pesticides.' },
              { icon: <ShieldCheck className="text-[#B38B59]" />, title: 'FSSAI Certified', desc: 'Full compliance with food safety standards. Verification available on FSSAI portal.' },
            ].map((item, i) => (
              <div key={i} className="bg-white p-8 rounded-3xl border border-stone-100 shadow-sm hover:shadow-xl transition-all duration-500 group">
      ...
      <section className="py-20 px-4 bg-white">
        <div className="max-w-7xl mx-auto">
          <div className="flex justify-between items-end mb-12">
            <div>
              <span className="text-[#B38B59] font-bold tracking-[0.3em] uppercase text-xs mb-4 block">Customer favorites</span>
              <h2 className="font-display text-3xl md:text-5xl font-bold text-[#42210B]">Best Sellers</h2>
            </div>
            <Link to="/shop" className="group flex items-center gap-2 text-[#B38B59] font-bold tracking-widest text-xs uppercase hover:text-[#42210B] transition-colors">
        ...
        <div className="max-w-7xl mx-auto">
          <div className="bg-[#42210B] rounded-[2rem] p-8 md:p-16 relative overflow-hidden flex flex-col md:flex-row items-center gap-12 text-white">
            ...
            <div className="flex-1 relative z-10 text-center md:text-left">
              <h2 className="font-display text-3xl md:text-5xl font-bold mb-6">Subscribe & Save 10%</h2>
              ...
              <div className="flex flex-wrap gap-6 justify-center md:justify-start">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm font-bold uppercase tracking-widest">Free Express Shipping</span>
                </div>
                <div className="flex items-center gap-3">
                  <RotateCcw className="text-[#B38B59]" size={20} />
                  <span className="text-sm font-bold uppercase tracking-widest">Flexible Scheduling</span>
                </div>
              </div>
            </div>
      ...
      <footer className="bg-[#1A0A00] text-stone-400 py-16 px-4">
        <div className="max-w-7xl mx-auto text-center">
          <div className="font-display text-2xl font-bold text-white mb-6 uppercase tracking-widest">Rathi Naturals</div>
          <p className="text-sm max-w-2xl mx-auto mb-12 leading-relaxed font-medium">
            Founded in 1965 in Sangaria, Rajasthan, Rathi Naturals has been bringing the purest spices 
            from the field to your kitchen for over three generations.
          </p>
