...
import { CheckCircle2, CreditCard, Truck, ShieldCheck, MapPin, Smartphone, Mail, User, ArrowLeft, ArrowRight } from 'lucide-react';

const CheckoutPage: React.FC = () => {
  const { cartItems: items, subtotal: totalPrice, cartItemCount: totalItems, clearCart } = useCart();
  ...
  if (step === 3) return (
    <div className="min-h-screen bg-[#FAF6F2] py-20 px-4">
      <div className="max-w-2xl mx-auto text-center">
        ...
        <h1 className="font-display text-4xl md:text-5xl font-bold text-[#42210B] mb-6">Order Confirmed!</h1>
        <p className="text-stone-500 text-lg mb-12 max-w-lg mx-auto leading-relaxed">
          Thank you, <span className="text-[#42210B] font-bold">{form.name}</span>. Your spices are being packed at our facility in Sangaria and will ship within 24 hours.
        </p>
        ...
        <div className="bg-white rounded-[2.5rem] border border-stone-100 shadow-xl p-8 md:p-12 text-left mb-12 relative overflow-hidden">
          ...
          <div className="space-y-4 mb-10">
            {items.map((item: CartItem) => (
              <div key={item.id} className="flex justify-between items-center text-sm">
                <span className="text-stone-600 font-medium">{item.name} <span className="text-stone-300 ml-1">({item.weight} × {item.quantity})</span></span>
          ...
          <div className="pt-8 border-t border-dashed border-stone-200 flex justify-between items-end">
            <div>
              <div className="text-[10px] font-black text-stone-300 uppercase tracking-widest mb-1">Total Paid</div>
              <div className="text-3xl font-bold text-[#42210B]">₹{grandTotal}</div>
            </div>
            <div className="text-right">
              <div className="text-[10px] font-black text-stone-300 uppercase tracking-widest mb-1">Status</div>
              <div className="text-sm font-bold text-green-600 uppercase tracking-widest">Processing</div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
            ...
            <div className="bg-white rounded-[2.5rem] border border-stone-100 shadow-xl p-8 md:p-12 overflow-hidden relative">
              ...
              {step === 1 && (
                <div className="space-y-10 relative z-10">
                  <h2 className="font-display text-3xl font-bold text-[#42210B]">Delivery Information</h2>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      ...
                      <input 
                        type="text" value={form.name} onChange={e => handleField('name', e.target.value)}
                      ...
                      />
                      {errors.name && <p className="text-[10px] text-red-500 font-bold uppercase tracking-wider mt-1 ml-2">{errors.name}</p>}
                    </div>
                    <div className="space-y-2">
                      <label className="text-[10px] font-black text-stone-400 uppercase tracking-widest flex items-center gap-2">
                      ...
                      <input 
                        type="tel" value={form.phone} onChange={e => handleField('phone', e.target.value)}
                      ...
                      />
                      {errors.phone && <p className="text-[10px] text-red-500 font-bold uppercase tracking-wider mt-1 ml-2">{errors.phone}</p>}
                    </div>
                    <div className="md:col-span-2 space-y-2">
                      ...
                      <input 
                        type="email" value={form.email} onChange={e => handleField('email', e.target.value)}
                      ...
                      />
                      {errors.email && <p className="text-[10px] text-red-500 font-bold uppercase tracking-wider mt-1 ml-2">{errors.email}</p>}
                    </div>
                    <div className="md:col-span-2 space-y-2">
                      <label className="text-[10px] font-black text-stone-400 uppercase tracking-widest flex items-center gap-2">
                      ...
                      <textarea 
                        rows={3} value={form.address} onChange={e => handleField('address', e.target.value)}
                      ...
                      />
                      {errors.address && <p className="text-[10px] text-red-500 font-bold uppercase tracking-wider mt-1 ml-2">{errors.address}</p>}
                    </div>
                    <div className="space-y-2">
                      <label className="text-[10px] font-black text-stone-400 uppercase tracking-widest">Pin Code</label>
                      <input 
                        type="text" value={form.pincode} onChange={e => handleField('pincode', e.target.value)}
                      ...
                      />
                      {errors.pincode && <p className="text-[10px] text-red-500 font-bold uppercase tracking-wider mt-1 ml-2">{errors.pincode}</p>}
                    </div>
                    <div className="space-y-2">
                      <label className="text-[10px] font-black text-stone-400 uppercase tracking-widest">City</label>
                      <input 
                        type="text" value={form.city} onChange={e => handleField('city', e.target.value)}
                      ...
                      />
                      {errors.city && <p className="text-[10px] text-red-500 font-bold uppercase tracking-wider mt-1 ml-2">{errors.city}</p>}
                    </div>
                  </div>
              ...
              {step === 2 && (
                <div className="space-y-10 relative z-10">
                  <div className="flex items-center gap-4 mb-2">
                    <button onClick={() => setStep(1)} className="p-2 hover:bg-stone-100 rounded-lg transition-colors text-stone-400">
                      <ArrowLeft size={20} />
                    </button>
                    <h2 className="font-display text-3xl font-bold text-[#42210B]">Payment Method</h2>
                  </div>

                  <div className="space-y-4">
                    {[
                      { id: 'upi', label: '📱 UPI (GPay, PhonePe, Paytm)', desc: 'Instant & Secure Payment' },
                      { id: 'card', label: '💳 Credit / Debit Card', desc: 'Secure card processing via Razorpay' },
                      ...
                      >
                        <div className="flex items-center gap-4">
                          <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${
                            payMethod === m.id ? "border-[#B38B59] bg-[#B38B59]" : "border-stone-200"
                          }`}>
                  ...
                  <div className="pt-8 space-y-4">
                    ...
                    <p className="text-center text-[10px] font-black text-stone-300 uppercase tracking-widest flex items-center justify-center gap-4">
                      ...
                      <span className="flex items-center gap-1 font-bold text-green-600"><CheckCircle2 size={12} /> ISO Certified</span>
                    </p>
                  </div>
                </div>
              )}
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-3xl border border-stone-100 p-8 shadow-sm">
              <h3 className="text-xs font-black text-stone-400 uppercase tracking-widest mb-8">Items in Cart ({totalItems})</h3>
                ...
                {items.map((item: CartItem) => (
                  <div key={item.id} className="flex gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-bold text-[#42210B] truncate mb-0.5">{item.name}</div>
                      <div className="text-[10px] text-stone-400 font-bold uppercase tracking-widest">{item.weight} × {item.quantity}</div>
                    </div>
                    <div className="text-sm font-bold text-[#42210B]">₹{item.price * item.quantity}</div>
                  </div>
                ))}
              </div>
              ...
              <div className="space-y-4 pt-6 border-t border-dashed border-stone-100">
                ...
                <div className="flex justify-between items-end pt-4">
                  <span className="text-xs font-black uppercase tracking-[0.2em] text-[#B38B59]">Grand Total</span>
            ...
            <div className="bg-[#B38B59]/5 border border-[#B38B59]/10 rounded-3xl p-6 flex items-center gap-4">
              ...
              <div>
                <div className="text-[10px] font-black text-[#B38B59] uppercase tracking-widest mb-1">Estimated Delivery</div>
                <div className="text-xs font-bold text-[#42210B]">Expected within 3-5 business days</div>
              </div>
            </div>
          </div>
        </div>
