...
import { SlidersHorizontal, ChevronDown, Search, Flame, Leaf, Box, Sparkles, LayoutGrid } from 'lucide-react';
  ...
  const filteredResults = useMemo(() => {
    return products
      .filter(p => {
        ...
        if (aCat === 'specialty') return pCat.includes('special') || pCat.includes('premium') || pCat.includes('saffron');
        return pCat.includes(aCat);
      })
      .filter(p => activeTags.length === 0 || activeTags.some(t => {
        if (t === 'Organic')       return p.tags?.some(tag => tag.toLowerCase().includes('organic'));
        if (t === 'Bestseller')    return p.badge === 'Bestseller' || p.tags?.some(tag => tag.toLowerCase().includes('bestseller'));
        if (t === 'Single origin') return p.badge === 'Single Origin' || p.tags?.some(tag => tag.toLowerCase().includes('single-origin'));
      ...
      <div className="bg-[#F2ECE4] border-b border-stone-200 py-3 px-4 md:px-8 shadow-inner overflow-x-auto no-scrollbar">
        <div className="max-w-7xl mx-auto flex items-center gap-4">
          <div className="flex items-center gap-2 pr-4 border-r border-stone-300">
            ...
            <span className="text-[10px] uppercase font-black tracking-[0.2em] text-[#B38B59]">Filters</span>
          </div>
          ...
          <div className="relative group">
            ...
            <ChevronDown size={14} className="absolute right-3 top-1/2 -translate-y-1/2 text-[#B38B59] pointer-events-none" />
          </div>
        </div>
      </div>
      ...
      <main className="max-w-7xl mx-auto px-4 md:px-8 py-10">
        ...
        {filteredResults.length > 0 ? (
          ...
          <div className="py-24 text-center bg-white rounded-[3rem] border border-stone-100 shadow-sm">
            ...
            <h2 className="font-display text-2xl font-bold text-[#42210B] mb-3">No products found</h2>
