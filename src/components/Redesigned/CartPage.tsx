  ...
  if (totalItems === 0)
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center px-4 bg-[#FAF6F2]">
        ...
        <h2 className="font-display text-3xl font-bold text-[#42210B] mb-4 text-center">
          Your Spice Box is Empty
        </h2>
        <p className="text-stone-500 mb-10 text-center max-w-sm leading-relaxed">
          Discover single-origin, cold-ground spices from Sangaria, Rajasthan. Freshly packed for
          your kitchen.
        </p>
        <Link
          to="/shop"
  ...
  return (
    <div className="bg-[#FAF6F2] min-h-screen pb-20">
      <div className="max-w-7xl mx-auto px-4 md:px-8 py-10">
        <h1 className="font-display text-3xl md:text-5xl font-bold text-[#42210B] mb-12">
          My Spice Box
        </h1>
                ...
                <div className="flex items-center gap-3">
                  ...
                  {remaining > 0 ? (
                    <span className="text-sm font-bold text-[#42210B]">
                      Add ₹<span className="text-orange-500">{remaining}</span> more for{' '}
                      <span className="text-orange-600 underline">FREE Express Shipping</span>
                    </span>
                  ) : (
            ...
            <div className="bg-white rounded-[2.5rem] border border-stone-100 shadow-sm overflow-hidden">
              <div className="divide-y divide-stone-100">
                {items.map((item: CartItem) => (
                  <div
                    key={item.id}
                    className="p-6 md:p-8 flex gap-6 group hover:bg-[#FAF6F2]/50 transition-colors"
                  >
                    {/* Image */}
                      ...
                      <div>
                        <div className="flex justify-between items-start mb-1">
                          <Link to={`/product/${item.productId}`}>
                            <h3 className="text-lg md:text-xl font-display font-bold text-[#42210B] group-hover:text-[#B38B59] transition-colors">
                              {item.name}
                            </h3>
                          </Link>
                          <span className="text-lg font-bold text-[#42210B]">
          ...
          <div className="space-y-6 lg:sticky lg:top-32">
            ...
            {suggestions.length > 0 && (
              <div className="bg-white rounded-3xl border border-stone-100 p-6 shadow-sm">
                <h3 className="text-xs font-black text-stone-400 uppercase tracking-widest mb-6 px-1">
                  Complete your box
                </h3>
                <div className="space-y-4">
                  {suggestions.map((p) => (
                    <Link
                      key={p.id}
                      to={`/product/${p.id}`}
                      className="group flex items-center gap-4 hover:bg-[#FAF6F2] p-2 rounded-2xl transition-colors"
                    >
                      ...
                      <div className="flex-1">
                        <div className="text-sm font-bold text-[#42210B] mb-0.5">
                          {p.shortName || p.name}
                        </div>
