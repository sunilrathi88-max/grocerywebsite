...
const generateSitemap = () => {
    ...
    while ((match = productRegex.exec(productsBlock)) !== null) {
        ...
        if (id < 100) { // Simple heuristic based on viewed data (products are 1-30, variants 100+)
            productIds.push(id);
        }
    }
  ...
  <url>
    ...
    console.log(`Sitemap generated with ${productIds.length} products, ${categories.size} categories, ${slugs.length} posts.`);
};
