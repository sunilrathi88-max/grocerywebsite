  ...
  switch (req.method) {
    case 'GET': {
      ...
      if (userId && typeof userId === 'string') {
        ...
        orders = await prisma.order.findMany({ include: { items: { include: { product: true } } } });
      }
    ...
    case 'POST': {
      ...
      if (!userId || !Array.isArray(items) || !paymentMethod || !shippingAddress) return res.status(400).json({ error: 'Missing required fields' });
